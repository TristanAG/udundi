<template>
  <div class="h-screen w-screen flex flex-col md:flex-row overflow-hidden md:h-screen">
  
    <div class="w-full md:w-1/3 bg-[#6A1B1A] text-white flex flex-col justify-center p-10 
                h-1/4 md:h-full">
      <h1 class="text-5xl md:text-7xl font-serif leading-tight mb-8">Explore</h1>
      <button 
        @click="openPanel" 
    ...
    <DetailsPanel :isVisible="isPanelOpen" @close="closePanel" />
...
<script setup>
import { ref } from 'vue';
import DetailsPanel from './components/DetailsPanel.vue'; // Adjust path if needed

// State to control panel visibility
const isPanelOpen = ref(false);

// Functions to open and close the panel
const openPanel = () => {
